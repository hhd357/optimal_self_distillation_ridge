...
# List of columns with "?" (missing values) to exclude
columns_to_exclude = [
    ...
    'PctPolicAsian', 'PctPolicMinor', 'OfficAssgnDrugUnits', 'NumKindsDrugsSeiz', 'PolicAveOTWorked', 'PolicCars',
    'PolicOperBudg', 'LemasPctPolicOnPatr', 'LemasGangUnitDeploy', 'PolicBudgPerPop'
]

df_clean = df.drop(columns=columns_to_exclude)
...
# Extract X and y
X = df_clean.drop(columns=['ViolentCrimesPerPop'])
y = df_clean[y_column]

np.random.seed(2026)

# Randomly split 5-95 into training and test sets
n = len(X)
...
ax.set_title('Communities and Crime', fontsize=ftsize + 4)
